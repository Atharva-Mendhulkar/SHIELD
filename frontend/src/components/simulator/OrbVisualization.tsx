...
export default function OrbVisualization({ score, phase, anomalyCount }: OrbVisualizationProps) {
  ...
  return (
    <div className="relative w-[350px] h-[350px] mx-auto my-8">
      ...
      >
        {phase === 'blocked' && <Lock className="w-6 h-6 text-white mb-1" />}
        {phase === 'allowed' && <UserCheck className="w-6 h-6 text-white mb-1" />}
        {phase === 'idle' && <Shield className="w-6 h-6 text-white/50 mb-1" />}
        {score !== null ? (
          <>
            <span className="font-mono text-2xl font-bold text-white">{score}</span>
            <span className="text-[10px] text-white/60 font-mono">SCORE</span>
          </>
        ) : (
          <span className="text-xs text-white/40 font-mono">READY</span>
        )}
      </div>
    </div>
  );
