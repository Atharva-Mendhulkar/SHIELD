import { SCENARIOS, type StepStatus } from './types';
...
export default function ScenarioSidebar({
  ...
  const stepBtnClass = (status: StepStatus) => {
    switch (status) {
      case 'pending': return 'border-border text-slate-500 cursor-not-allowed opacity-40';
      case 'ready': return 'border-shield-green text-foreground cursor-pointer hover:shadow-[0_0_15px_rgba(0,255,136,0.3)]';
      case 'active': return 'border-shield-amber text-foreground cursor-wait animate-pulse';
      case 'done': return 'bg-shield-green/20 border-shield-green text-shield-green cursor-default';
    }
  };

  return (
    <aside className="w-[300px] min-w-[300px] bg-card border-r border-border flex flex-col h-full overflow-y-auto">
      <div className="p-4">
          ...
          {SCENARIOS.map((s) => (
            <button
              ...
              className={`w-full text-left p-2.5 rounded transition-all border ${
                activeScenario === s.id
                  ? 'border-l-[3px] border-l-shield-green border-t-border border-r-border border-b-border bg-secondary'
                  : 'border-border hover:bg-secondary/50'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs text-slate-500">{String(s.id).padStart(2, '0')}</span>
