...
LEGITIMATE_PROFILE = {
    ...
    # Device Trust Context -- legitimate user on known mobile
    "device_class_known": 1,
    "device_session_count": (15, 5),
    "device_class_switch": 0,
    "is_known_fingerprint": 1,
    "time_since_last_seen_hours": (24, 12),
    # Desktop Mouse -- zeros for mobile user
...
def generate_legitimate_session(user_id: int):
    ...
    for name in FEATURE_NAMES:
        if name not in features:
            if "std" in name:
                features[name] = np.random.uniform(5, 8)
            elif "mean" in name:
                features[name] = np.random.uniform(140, 160)
            elif "count" in name:
